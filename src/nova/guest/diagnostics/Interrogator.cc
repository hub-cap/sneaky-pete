...
DiagInfoPtr Interrogator::get_diagnostics() {
    ...
    DiagInfoPtr retValue(diagnostics=new DiagInfo());
    pid_t pid = getpid();

    ProcStatus * status = dynamic_cast<ProcStatus *>(diagnostics);
    get_proc_status(pid, *status);

    diagnostics->system_info.num_cpus = get_num_cpus();
    diagnostics->system_info.mem_total = get_mem_total();

    // Add the version to the Info
    diagnostics->version = NOVA_GUEST_CURRENT_VERSION;
